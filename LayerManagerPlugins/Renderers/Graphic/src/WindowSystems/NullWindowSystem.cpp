...
void NullWindowSystem::printDebug(int posx, int posy)
{
    ...
    for(std::list<Layer*>::const_iterator currentLayer = list.begin(); currentLayer != list.end(); ++currentLayer)
    {
        ...
        for(std::list<Surface*>::iterator current = (*currentLayer)->m_surfaces.begin(); current != (*currentLayer)->m_surfaces.end(); ++current)
        {
...
//    LOG_INFO("NullWindowSystem","redraw");
    ...
    for(std::list<Layer*>::const_iterator current = layers.begin(); current != layers.end(); ++current)
    {
        ...
        if ((*currentLayer).visibility && (*currentLayer).opacity>0.0f)
        {
            ...
            std::list<Surface*> surfaces = currentLayer->m_surfaces;
