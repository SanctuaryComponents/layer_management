...
class MockCommandExecutor : public CommandExecutor {
 public:
  MOCK_METHOD1(execute, bool(Command* commandToBeExecuted));

  MOCK_METHOD1(getLayerTypeCapabilities, uint(LayerType));
  MOCK_METHOD1(getNumberOfHardwareLayers, uint(uint));
  MOCK_METHOD1(getScreenResolution, uint*(uint));
  MOCK_METHOD1(getScreenIDs, uint*(uint*));
};
